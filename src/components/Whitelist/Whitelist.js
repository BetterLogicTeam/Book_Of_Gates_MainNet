...
function Whitelist() {
  ...
  const Mint_Nft=async()=>{
    try {
      ...
      let contractOf= new web3.eth.Contract(NFT_Contract_ABI,NFT_Contract)
      let getValue= await contractOf.methods.firstPreSaleCost().call()
      let publicSaleDate= await contractOf.methods.publicSaleDate().call()
      let gettotalsupply= await contractOf.methods.totalSupply().call()
      let getmaxsupply= await contractOf.methods.maxSupply().call()
      let getfirstMaxMintAmountPresale= await contractOf.methods.firstMaxMintAmountPresale().call()
      ...
      if(Number(now)<Number(publicSaleDate)){
        if(Number(newtotal)<=Number(getmaxsupply)){
          if(Number(ethbalance)>=Number(Value)){
            ...
            if(Number(count)<=Number(getfirstMaxMintAmountPresale)){
      await contractOf.methods.whiteListMint(count,getProof).send(
        {
          from:acc,
          value:Value
        }
  ...
  const Completionist = () => {
    return (
      <>
        <div className="text_days fs-5 ">Whitelist Presale Started</div>
      </>
    );
  };
    ...
    if (completed) {
      ...
      return (
        <div className="text_days fs-5 ">
          ...
          remaining until Whitelist Presale starts
              ...
              <div class="row">
                <div class="col-md-7 left_connent text-start">
                  <h1 class="main_home_heading1 text-white">
                    Welcome to the Whitelist Presale of
                    <br />
                ...
                <div class="col-md-5 right_coonent mt-5 mt-md-0">
                  <div class="right_content_card">
                    <h4 class="card_heading_span pt-3">Whitelist Presale</h4>
                    <div className="text_days fs-5 ">
                  <Countdown
                    date={
                      Date.now() + (parseInt("1679885224") * 1000 - Date.now())
                    }
                    renderer={renderer}
                  />
                </div>
